# ============================================================
# 🏀 AGENT IA NBA — OVER/UNDER TOTAL + PAR ÉQUIPE
# ============================================================
...
def get_team_stats(team_id, num_games=20):
    ...
    Inclut total du match ET stats par équipe.
    """
    try:
        resp = requests.get(f"{BASE_URL}/games", params={
            "team_ids[]": team_id,
        ...
        return {
            ...
            "max_pts":      max(totals),
            "min_pts":      min(totals),
            # ── Stats par équipe (marché individuel) ──
            "over120_pct":  round(sum(1 for p in pts_scored if p > 120) / n * 100, 1),
            "over115_pct":  round(sum(1 for p in pts_scored if p > 115) / n * 100, 1),
            "over110_pct":  round(sum(1 for p in pts_scored if p > 110) / n * 100, 1),
            "over105_pct":  round(sum(1 for p in pts_scored if p > 105) / n * 100, 1),
            "max_scored":   max(pts_scored),
            "min_scored":   min(pts_scored),
        }
...
── PAR ÉQUIPE ──
- Pts marqués (moy.)           : {home_stats['avg_scored']}
- Over 120.5 pts               : {home_stats['over120_pct']}% des matchs
- Over 115.5 pts               : {home_stats['over115_pct']}% des matchs
- Over 110.5 pts               : {home_stats['over110_pct']}% des matchs
- Over 105.5 pts               : {home_stats['over105_pct']}% des matchs
- Min / Max marqués            : {home_stats['min_scored']} / {home_stats['max_scored']}
...
── PAR ÉQUIPE ──
- Pts marqués (moy.)           : {away_stats['avg_scored']}
- Over 120.5 pts               : {away_stats['over120_pct']}% des matchs
- Over 115.5 pts               : {away_stats['over115_pct']}% des matchs
- Over 110.5 pts               : {away_stats['over110_pct']}% des matchs
- Over 105.5 pts               : {away_stats['over105_pct']}% des matchs
- Min / Max marqués            : {away_stats['min_scored']} / {away_stats['max_scored']}
...
def lancer_analyse():
    ...
    if retenus:
        ...
        send_telegram(
            ...
            "Pour chaque match :\n"
            "1. Total match — ligne recommandée + confiance\n"
            "2. Par équipe — ligne recommandée pour chaque équipe + confiance\n"
            "3. Points forts et risques\n"
            "4. Tes meilleures sélections pour un combiné"
        )
    else:
